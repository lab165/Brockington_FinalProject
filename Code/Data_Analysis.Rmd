...
aic_all <- lm(data = daily_bat_discharge, TotalCalls ~ DailyTemp + TallVegByMSU + LunarPhase + TotalPreyRate + discharge)
step(aic_all)

#running multiple regression on variables from AIC
lm_all <- lm(data = daily_bat_discharge, TotalCalls ~ DailyTemp + TallVegByMSU + LunarPhase + TotalPreyRate + discharge)
summary(lm_all)
