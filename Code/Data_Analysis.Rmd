...
discharge_plot <- #plotting ozone concentration over time
...
aic_all <- lm(data = daily_bat_discharge, TotalCalls ~ DailyTemp + Site + LunarPhase + TotalPreyRate + discharge)
step(aic_all)

#running multiple regression on variables from AIC
lm_all <- lm(data = daily_bat_discharge, TotalCalls ~ DailyTemp + Site + LunarPhase + discharge)
summary(lm_all)
